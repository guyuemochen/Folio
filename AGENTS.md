...
本项目（Folio）的工作流规范。**所有贡献者（含 AI agent）在动手前必须阅读本文件。**

包含两部分：
- **§0 任务并行化原则** —— 拆分 + 并行优先（适用于所有开发与修复）
- **§1–§8 Git 工作流** —— 分支 / 提交 / 发版 / Tag

---

## 0. 任务并行化原则（核心工作流）

**默认假设：任何更新或修复都应该并行完成，而不是线性串行。**

动手前先问自己：**「这件事能不能拆成几个互相独立的子任务？」** 能拆就拆开并行；只有存在真正的依赖或强冲突时才串行。线性地一个接一个做是被禁止的默认模式（见 §8）。

### 0.1 什么样的任务可以并行

满足以下任一条件即可并行：
- **无文件依赖** —— 两个任务改的是不同的模块 / 文件
- **无逻辑依赖** —— 任务 B 不需要任务 A 的输出才能开始
- **独立可验证** —— 每个子任务可以单独 `pnpm typecheck` / `pnpm build` / 测试

### 0.2 怎么拆

1. **按模块 / 文件边界拆** —— `editor`、`db`、`ai-assistant`、`i18n`、`settings`、`search`、`import-export` 等都是天然的拆分边界
2. **按层拆** —— 前端 (`src/`) / Rust 核心 (`src-tauri/`) / 类型定义 / 测试 通常可以并行推进
3. **按修复点拆** —— 多个不相关的 bug **永远不要**塞进同一个 commit 或同一个分支，哪怕都很小

### 0.3 怎么并行执行

- **每个并行任务一个 feature 分支**（命名见 §1.2），互不阻塞，分别合并到 `dev`
- **多 agent / 多 PR 并行推进**，让独立任务真正同时跑起来，而不是排队等上一个完成
- **修改同一文件的多个任务必须串行**或显式协调（后开的分支 rebase 解决冲突）

### 0.4 什么时候不要并行

- 任务之间有强依赖（B 必须读到 A 的输出）
- 改动会触发跨模块的大规模重构（先讨论方案再拆）
- 冲突成本 > 并行收益（典型如同时改同一个核心文件，如 `db.rs` / `App.tsx` / `lib.rs`）
...
| `dev` | **开发分支**。所有功能分支的集成分支，保持最新开发进度。 | 允许直接合并；功能分支也可通过 PR 合并。 |

### 1.2 功能分支（短期）

**每项开发任务必须单独创建分支**，完成后合并入 `dev`（可直接 merge，也可走 PR）。
...
- ❌ 把多个互相独立的任务**串行**处理（应拆分为并行 feature 分支，见 §0）。
- ❌ 把多个不相关的 bug 或功能塞进同一个分支 / 同一个 commit（见 §0.2）。
