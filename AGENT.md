...
| 远端 | 地址 | 用途 |
|------|------|------|
| `origin` | `https://gitlab.guyuemochen.tech/root/folio.git` | **主仓库**。所有开发、PR、代码评审在此进行。 |
| `github` | `https://github.com/guyuemochen/Folio.git` | **镜像仓库**。仅供 CI 发布流水线使用（GitHub Releases）。不要直接在 GitHub 上开发。 |

- 代码提交一律推送到 **GitLab (`origin`)**，由 GitLab 配置的 Push Mirror 自动同步到 GitHub。
- GitHub Actions（`.github/workflows/release.yml`）在镜像仓库上运行，负责构建并发布到 GitHub Releases。
...
| `hotfix` | 生产紧急修复（从 `master` 拉出，见 §4） | `hotfix/crash-on-startup` |
...
- ❌ 直接 push 到 `github` 远端（开发只在 GitLab）。
